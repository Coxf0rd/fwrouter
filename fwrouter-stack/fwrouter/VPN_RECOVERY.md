# Восстановление VPN (fwrouter + mihomo)

Ниже — рабочая, проверенная схема поднятия fwrouter (FastAPI UI) + mihomo
для корректной подписки и полного маршрута через TUN. Этот файл нужен
для быстрого восстановления, если VPN «упал».

## GOAL
- Поднять fwrouter (FastAPI UI) + mihomo (Clash Meta) так, чтобы:
...
- Для “FULL HOST ROUTING” контейнер mihomo должен быть в network_mode: host.
...
- Конфиг лежит на хосте: `/etc/fwrouter/mihomo/config.yaml`
  (если у тебя отдельная «вторая копия», то может быть `/etc/fwrouter/mihomo2/config.yaml`).
...
- Файл (обычно): `/app/fwrouter/docker-compose.mihomo.yml`
  (для второй копии: `/app/fwrouter/docker-compose.mihomo2.yml`)
...
Пример docker-compose (см. файлы в репо):
---
services:
  mihomo:
    ...
...
Запуск:
- `cd /app/fwrouter`
- базовый вариант: `docker compose -f docker-compose.mihomo.yml up -d --force-recreate`
- вариант со второй копией: `docker compose -f docker-compose.mihomo2.yml up -d --force-recreate`
...
- Если provider пустой или исчез:
  ...
  - в логах mihomo2 ищи DNS/EOF/403:
    docker logs --tail 200 fwrouter-mihomo-2 || docker logs --tail 200 fwrouter-mihomo
...
то меняем `external-controller` в конфиге Mihomo (ОСТОРОЖНО: открывает API в сеть):
```bash
sed -i 's/^external-controller: .*/external-controller: 0.0.0.0:9191/' /etc/fwrouter/mihomo/config.yaml
```
3) Перезапустил контейнер, чтобы Mihomo2 перечитал конфиг:
```bash
docker restart fwrouter-mihomo-2 || docker restart fwrouter-mihomo
```
