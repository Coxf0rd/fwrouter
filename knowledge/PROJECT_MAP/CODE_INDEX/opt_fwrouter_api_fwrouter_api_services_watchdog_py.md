...
- `detect_recent_vpn_traffic_attempts(...)` reads `traffic_counter_snapshots` for `path='vpn'` and returns aggregate `total_rx_delta`/`total_tx_delta` plus `response_observed`, `outbound_observed`, and `traffic_stalled`.
