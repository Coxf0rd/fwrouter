...
- In automatic mode, response bytes (`rx_delta > 0`) prove the VPN path is alive. Latency and delay-check results may be collected as UI/health/ranking evidence, but they never trigger automated recovery or selector failover. Automated recovery is driven only by confirmed traffic stall observations.
