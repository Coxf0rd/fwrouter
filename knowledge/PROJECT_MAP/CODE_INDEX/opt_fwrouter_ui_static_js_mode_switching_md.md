...
- The admin external-network tab respects both generic `system_visibility.external_network_source` and concrete discovered source visibility such as `external-network-tailscale`; hiding a concrete source in `Connections` must hide its admin tab/rows too.
