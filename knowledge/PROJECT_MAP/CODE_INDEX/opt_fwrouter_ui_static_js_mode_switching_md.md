...
- Admin Devices loads lightweight settings inventory for Lan, external network clients, Vless clients, Docker, and Host; optional tabs use i18n labels and are hidden after the first inventory load when the corresponding inventory class has no rows or is disabled by `system_visibility`.
- Settings inventory calls `/ui/settings/inventory` with `include_inactive=true`, so inactive Vless subscription groups and other manageable objects remain visible in Settings even when Admin hides inactive rows.
