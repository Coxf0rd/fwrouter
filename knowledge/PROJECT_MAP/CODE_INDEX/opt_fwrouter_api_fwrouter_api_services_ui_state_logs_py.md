...
- `ui_visible` is intentionally permissive for normal events: default UI journal requests should show summarized operational/technical events returned by the log endpoint, except explicit no-op technical watchdog statuses.
...
- Watchdog technical events stay complete in JSONL storage. Actionable/pending/failure/failover rows remain visible in the UI journal after summarization, while no-op statuses such as `paused_signal_unavailable`, `no_failure_no_traffic`, and `active_quality_degraded_traffic_healthy` return `ui_visible=false`.
...
- `external_client.deleted` and `external_client.delete_failed` are known
  lifecycle events for VLESS/Xray external-client cleanup.
