...
- Keep operator-facing text in `UI_TEXT_REGISTRY`: stable namespaces such as `watchdog.status`, `watchdog.action`, and `error.code` map backend machine keys to `title`/`reason` strings. Unknown keys must use a localized fallback and keep the raw machine code in details for diagnostics.
