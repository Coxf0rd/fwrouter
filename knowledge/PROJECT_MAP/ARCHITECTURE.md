...
- Runtime adapter selection is role-based: `runtime_adapters.py` maps roles such as `vpn_dataplane` and `explicit_client_runtime` to registered adapter implementations and capabilities. Logical health, ping, effective-member observation, and selector checks use the active `vpn_dataplane` operations from this registry. An adapter with native logical-group state plus a refresh operation owns health probing for its paths; otherwise the existing local delay probe is the only probe backend.
...
- Backend startup starts two runtime health lanes: active observation runs about
  every 60 seconds for the current active logical server's effective member
  path, while the bounded
...
- The events framework keeps typed audit/operational/diagnostic models compatible with legacy writers while centralizing a versioned event envelope, recursive credential sanitization and bounded payload metadata. HTTP request IDs flow through contextvars; `JobManager` explicitly snapshots the safe context in a separate job column and restores it in worker threads. Technical JSONL records retain event IDs, with deterministic IDs for legacy records lacking one. API startup re-scrubs configured operational and technical JSONL after database bootstrap so cleanup still runs when an installer has already advanced the schema without service log-path configuration.
- The diagnostic framework aggregates SQLite schema/integrity checks, state projection, reconcile results, typed event summary, and separately labeled historical technical failures/recovery events into the read-only `GET /api/v2/diagnose`/`fwrouter diagnose` report. Historical failures do not change current health status.
- Typed event DTOs and direct typed writer calls require a non-empty `event_code`; the envelope is schema version 2 and the initial Core catalog covers common audit/apply, health, routing, and recovery codes. It carries category, severity, component, and correlation context. Legacy writers and rows retain `event_type` as an explicitly marked compatibility code without a database migration. UI titles resolve through code and structured reason/error dictionaries, with text translation limited to marked legacy projections. Future Mihomo, Xray, and Tailscale events remain namespaced within their module boundaries.
- Public `/health` and `/system/summary` responses omit local filesystem paths and SQLite exception text. Safe stable error codes/messages are returned publicly; exception details remain in server-side diagnostics.
