...
- Runtime adapter selection is role-based: `runtime_adapters.py` maps roles such as `vpn_dataplane` and `explicit_client_runtime` to registered adapter implementations and capabilities. Selector core uses the active `vpn_dataplane` adapter interface instead of importing Mihomo directly.
