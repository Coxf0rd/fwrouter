...
- Settings inventory has a first-level `Подключения` tab backed by the generic systems list from backend state. `managed` means FWRouter owns lifecycle, `external` means user-managed service, `inventory` means view-only discovered host/container objects, and UI visibility means show/hide in admin only.
- Custom external connections in settings are created through the `Добавить подключение` dialog and store purpose (`external_management`, `external_vpn_module`, `external_network_source`), location, address, optional runtime type, endpoints and capabilities. The UI shows one copyable JSON mounting/API contract for the selected purpose, including available fields and probe/response shape when relevant.
