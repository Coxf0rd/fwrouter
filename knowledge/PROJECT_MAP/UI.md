...
- Settings journal uses compact typed `/api/v2/events/recent?view=summary`: the main journal shows Audit and Operational events, while Diagnostic events are isolated in the advanced diagnostic-events tab. Each event remains a separate row with an absolute timestamp; repeated-count presentation does not replace source events. During source/live version skew, legacy `/logs/operational` and `/logs/technical` remain compatibility fallbacks.
