...
- Из `решения/` удалены неканонические исторические `log-*`, backup index, prompt draft и старый traffic requirements draft; архитектурная карта теперь хранит только актуальные docs/ADR/CODE_INDEX.
- Из `/opt/fwrouter-api` удалены старые rollout/acceptance/report markdown, которые ссылались на прежний `read-server` layout и дублировали актуальные `решения/`. Оставлен отдельный рабочий `CONTROL_PLANE_TRANSFER.md`.
...
- UI style guide перенесен из `/opt/fwrouter-ui/static/css/CSS_BLOCKS.md` в `/решения/UI.md`, чтобы static tree содержал runtime assets, а документация оставалась в knowledge map.
