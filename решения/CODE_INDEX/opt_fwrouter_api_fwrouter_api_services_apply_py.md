...
- subject-only fast apply на фоне `global=direct` использует lighter verify path: пропускает full global live probe и вместо этого проверяет live `fwrouter_classify` на presence expected scoped rule для конкретного subject
- но для `domain-aware selective` даже fast subject apply все равно обязан прогонять `dnsmasq reconcile`, потому что `dataplane-apply.sh` пересоздает `inet fwrouter_v2`, а живой `dnsmasq nftset` runtime иначе остается со stale set references
...
- `fast_subject_apply` ускоряет именно verify/read-model path; `dnsmasq` lifecycle он не обходит для `domain-aware selective`, иначе после `nft delete table inet fwrouter_v2` `dnsmasq` начинает писать в несуществующие `nftset` объекты
