...
Этот репозиторий предназначен для установки стэка на **домашний шлюз/роутер** на Linux, который является входной точкой интернета и раздаёт его в локальную сеть.
...
## Про \"нельзя\" / что понадобится дополнительно (особенно для vless-gateway)

### Если используешь только базу (fwrouter + mihomo2)

- **Белый IP и домен не обязательны**, если ты будешь заходить в UI только из локальной сети.
- Нужен LAN‑доступ к шлюзу (например, `http://192.168.0.1:9280/`).

### Если включаешь vless-gateway (доступ извне)

Обычно понадобится:

- **Белый IP** на стороне шлюза *или* проброс портов (port-forward) до него.
- **Домен** (A/AAAA запись на твой IP) — чтобы нормально работал TLS и чтобы клиентам было куда подключаться.
- Открытые/проброшенные порты:
  - `XRAY_PORT` (по умолчанию `8443`) — VLESS+REALITY
  - `443/tcp` — если используешь WS+TLS через reverse‑proxy (NPM/nginx) и путь `/vless`
  - `SUB_PORT` (по умолчанию `18080`) — если хочешь раздавать подписки напрямую наружу
- TLS‑сертификат/ключ (если используешь WS+TLS) — это уже **секреты**, их в Git хранить нельзя.
- REALITY ключи — это **секреты**, их в Git хранить нельзя.
