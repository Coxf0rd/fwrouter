...
class _VersionOnlyBigVpnRulesSourceAdapter:
    def __init__(self) -> None:
        self.full_big_vpn_fetches = 0
    ...
    def fetch_big_vpn_source_versions(self) -> RulesSourcePayload:
        return RulesSourcePayload(
            values=[],
    ...
    def fetch_big_vpn_sources(self) -> RulesSourcePayload:
        self.full_big_vpn_fetches += 1
        raise AssertionError("full big_vpn fetch should be skipped for unchanged source versions")
...
def test_rules_full_update_skips_big_vpn_download_when_git_version_is_current(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _configure_env(monkeypatch, tmp_path)
    ...
    version_only_adapter = _VersionOnlyBigVpnRulesSourceAdapter()
    monkeypatch.setattr(
        "fwrouter_api.services.rules.DEFAULT_RULES_SOURCE_ADAPTER",
        version_only_adapter,
    )
    second_result = run_rules_full_update(_create_job())

    assert first_result["job_status"] == "success"
    assert second_result["job_status"] == "success"
    assert second_result["stage"] == "version_noop"
    assert second_result["changed"] is False
    assert version_only_adapter.full_big_vpn_fetches == 0
