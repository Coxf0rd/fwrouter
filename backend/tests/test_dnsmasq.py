...
def test_reconcile_dnsmasq_rules_force_restarts_without_config_change(
    monkeypatch,
    tmp_path: Path,
) -> None:
    clear_live_probe_cache()
    ...
    first_result = dnsmasq_service.reconcile_dnsmasq_rules()
    calls.clear()

    result = dnsmasq_service.reconcile_dnsmasq_rules(force_restart_reason="nft_table_recreated")

    assert first_result["restart_reason"] == "config_changed"
    assert result["ok"] is True
    assert result["rules_changed"] is False
    assert result["dhcp_dns_changed"] is False
    assert result["ipv6_lan_changed"] is False
    assert result["local_hosts_changed"] is False
    assert result["restart_required"] is True
    assert result["restart_reason"] == "nft_table_recreated"
    assert [argv[:3] for argv in calls] == [["systemctl", "restart", "dnsmasq"]]
