...
def test_fresh_database_starts_at_current_schema(monkeypatch, tmp_path: Path) -> None:
    ...
    assert schema_state["actual_schema_version"] == str(migrations.CURRENT_SCHEMA_VERSION)
    with _connect_raw() as connection:
        rows = {
...
def test_schema_10_upgrade_to_current_preserves_intent_and_is_bootstrap_idempotent(
    ...
    assert first_bootstrap["database_schema"]["actual_schema_version"] == str(migrations.CURRENT_SCHEMA_VERSION)
    ...
    assert second_bootstrap["database_schema"]["actual_schema_version"] == str(migrations.CURRENT_SCHEMA_VERSION)
