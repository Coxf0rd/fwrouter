...
def probe_external_ingress_runtime(provider: str) -> dict[str, Any]:
    contract = _provider_contract(provider)
    ...
    if not script_id:
        return {
            ...
            "details": {"script_available": False, "peers_visible_count": 0, "importable_peers_count": 0},
        }

    cache_key = str(probe_config.get("cache_key") or f"external_ingress.runtime.{provider}")
    return get_live_probe_cache(
        cache_key,
        ttl_seconds=float(probe_config.get("ttl_seconds") or 5.0),
        loader=lambda: _probe_external_ingress_runtime_uncached(provider, script_id),
    )


def _probe_external_ingress_runtime_uncached(provider: str, script_id: str) -> dict[str, Any]:
    contract = _provider_contract(provider)
    mapping = dict(contract.get("status_mapping") or {})
    ...
    peers = _peer_items(payload, mapping)
    clients = external_ingress_clients_from_payload(provider, payload)
